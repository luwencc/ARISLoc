...
def train_ppo(cfg: PPOConfig) -> None:
    ...
    print(f'[init] traj index: {n_ok} folders across {len(MOVE_TO_DRL_RPS)} RPs ({time.perf_counter() - t_init:.1f}s)', flush=True)
    ...
    ep_mode = f'path1 random csv x phase0-5/RP, sub-eps/ep <={n_path1_per_ep}' if cfg.enumerate_path1_train and (not cfg.synthetic_rss) else f'RPs/episode={len(MOVE_TO_DRL_RPS)}'
    ...
    if cfg.enumerate_path1_train and (not cfg.synthetic_rss):
        print(f'[init] episode 1 queue: {len(env._path1_tasks)} path1 sub-episodes (≤{n_path1_per_ep} = {len(MOVE_TO_DRL_RPS)} RP × {cfg.num_phases} phase) ({time.perf_counter() - t_reset:.1f}s)', flush=True)
    ep_return = 0.0
        ...
        try:
            ...
            if cdf_errs and save_error_cdf_plot(cdf_errs, cdf_path, xlim_max_m=float(CDF_XLIM_MAX_M), label='PPO DRL (path1 csv × trained policy)'):
                ...
                try:
                    ...
                    save_cdf_bundle('drl_v2', cdf_errs, depths=[1] * len(cdf_errs), meta={'script': 'PPORSLoc.py', 'eval_scope': 'path1 csv only'})
