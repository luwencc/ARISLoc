#!/usr/bin/env python3
"""Frozen SMSR MixedModel inference used as the localization oracle."""
...
_SMSR_ROOT = _Path(__file__).resolve().parents[2] / "SMS-R"
if str(_SMSR_ROOT) not in sys.path:
    sys.path.insert(0, str(_SMSR_ROOT))
import SMSR as loc  # noqa: E402
from .caches import LocInferResultCache
from .config import (EVAL_SOFTMAX_DENOM_SCALE, EVAL_TOPK_PROB_MASS_DIV, EVAL_TOPK_WEIGHT_MODE, LOC_INFER_BATCH_SIZE, LOC_INFER_CACHE_ENABLED, LOC_INFER_CACHE_MAX_ENTRIES, NUM_PHASES, OBS_GRID_ZSCORE, PPOConfig, default_ppo_config)
from .traj import parse_traj_folder_name

def apply_drl_localizer_eval_config(*, softmax_denom_scale: Optional[float]=None, topk_weight_mode: Optional[str]=None, prob_mass_div: Optional[float]=None) -> None:
    loc.apply_eval_localization_config(softmax_denom_scale=float(softmax_denom_scale) if softmax_denom_scale is not None else float(EVAL_SOFTMAX_DENOM_SCALE), topk_weight_mode=str(topk_weight_mode) if topk_weight_mode is not None else str(EVAL_TOPK_WEIGHT_MODE), prob_mass_div=float(prob_mass_div) if prob_mass_div is not None else float(EVAL_TOPK_PROB_MASS_DIV))
...
def load_localizer(cfg: PPOConfig) -> Tuple[nn.Module, np.ndarray, np.ndarray, pd.DataFrame, torch.Tensor, torch.Tensor]:
    ...
    denom = float(loc._eval_softmax_denom_scale())
    ...
    print(f'[DRL localizer] aligned with Test_Eval | Top-{int(loc.Config.K_VALUE)} weight_mode={wt_mode} softmax_denom_scale={denom:.2f} prob_mass_div={mass_div:.2f} fuse_xy lam={lam:.2f} valid_classes={int(valid_mask.sum().item())}/{len(loc.Config.RP_LIST)} ({('mask from ' + vc_json if Path(vc_json).is_file() else 'no mask file; all classes valid')})')
