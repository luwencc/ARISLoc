...
def _eval_softmax_denom_scale() -> float:
    s = float(getattr(Config, 'EVAL_SOFTMAX_DENOM_SCALE', 1.0) or 1.0)
    return max(s, 1e-06)
...
def apply_eval_localization_config(*, softmax_denom_scale: Optional[float]=None, topk_weight_mode: Optional[str]=None, prob_mass_div: Optional[float]=None) -> None:
    if softmax_denom_scale is not None:
        Config.EVAL_SOFTMAX_DENOM_SCALE = max(float(softmax_denom_scale), 1e-06)
...
def _weighted_rp_coords_from_logits(outputs, coord_df, batch_idx: int=0):
    ...
    denom_scale = _eval_softmax_denom_scale()
...
def _evaluate_localization_batched(model: nn.Module, coord: pd.DataFrame, lens: np.ndarray, sequences_padded: np.ndarray, true_rp_names: np.ndarray, true_xy: np.ndarray, mean: np.ndarray, std: np.ndarray, use_amp: bool, official_rp_set: Set[str], valid_mask: torch.Tensor, depths: Optional[np.ndarray]=None) -> Tuple[List[float], List[float], List[float], int, int, List[str], List[int]]:
    ...
    mass_div = _eval_topk_prob_mass_div()
    print(f'[eval] test batch_size={bs} dataloader_workers={nw} | Top-{int(Config.K_VALUE)} weight_mode={wt_mode} softmax_denom_scale={denom_scale:.2f} prob_mass_div={mass_div:.2f} + fuse_xy lam={lam:.2f} (valid masked classes={int(vm.sum().item())}）')
...
def save_validation_diagnostic_plots(val_loader: DataLoader, device: torch.device, use_amp: bool, num_classes: int, rp_list: Sequence[str]) -> None:
    ...
    ax_a.set_xlabel('RP index (same order as RP_LIST)', fontsize=11)
